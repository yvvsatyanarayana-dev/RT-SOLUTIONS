...
def insights():
    ...
    posts = BlogPost.query.order_by(BlogPost.created_at.desc()).paginate(page=page, per_page=9)
    return render_template('pages/blog.html', posts=posts)

@app.route('/blog/<slug>')
def blog_post(slug):
    post = BlogPost.query.filter_by(slug=slug).first_or_404()
    recent_posts = BlogPost.query.filter(BlogPost.id != post.id).order_by(BlogPost.created_at.desc()).limit(3).all()
    return render_template('pages/blog-detail.html', post=post, recent_posts=recent_posts)
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        return jsonify({'status': 'error', 'message': 'Invalid credentials or account disabled'}), 401
...
def admin_dashboard():
    ...
    return render_template('admin/dashboard.html', stats=stats, recent_contacts=recent_contacts, admin=admin)
...
def admin_contacts():
    ...
    contacts = query.order_by(Contact.created_at.desc()).paginate(page=page, per_page=20)
...
def admin_clients():
    page = request.args.get('page', 1, type=int)
    clients = Client.query.order_by(Client.created_at.desc()).paginate(page=page, per_page=20)
...
def manage_client(client_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            client.contract_value = float(data.get('contract_value', client.contract_value))
...
def init_database():
    with app.app_context():
        ...
        if Service.query.count() == 0:
            services = [
                Service(name='Web Development', description='Custom web applications', price=5000),
                Service(name='Mobile Development', description='Native and cross-platform apps', price=8000),
                Service(name='Cloud Solutions', description='AWS and Azure deployment', price=3000),
                Service(name='Consulting', description='Expert technical consultation', price=2000),
            ]
